          ...
          (let ([sel (send lst get-selection)])
            (unless (equal? #f sel)
              (let*([ltr (send lst get-data sel)]
                ...
                (send mytxt2 clear)
                (unless (equal? #f knf-grade)
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "Grade:") eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f))
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "\t~a~n" knf-grade) eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f)))
                (unless (equal? #f knf-strokenum)
                  (let ([eb (send mytxt2 last-position)])
                ...
                (unless (equal? #f knf-variant)
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "Variants:") eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f))
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "\t~a~n" knf-variant) eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f)))
                (unless (equal? #f knf-freq)
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "Usage Freq:") eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f))
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "\t~a~n" knf-freq) eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f)))
                (unless (equal? #f knf-jlpt)
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "JLPT:") eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f))
                  (let ([eb (send mytxt2 last-position)])
                    (send mytxt2 insert (format "\t~a~n" knf-jlpt) eb)
                    (send mytxt2 change-style sty2-normal eb 'end #f)))
                
                (unless (equal? #f knf-readings)
                  (let ([eb (send mytxt2 last-position)])
